    ...
    <form onSubmit={() => onSubmit()}>
      ...
      <Box sx={{ display: 'grid', gap: 2 }}>
        ...
        <FormControl>
          ...
          <Input type="text" placeholder="не указан" value={address || ''} disabled />
          <FormHelperText>
            {address ? '' : 'Будет указан после деплоя вестинг-контракта из предыдущей вкладки'}
          </FormHelperText>
        </FormControl>
        <Button
          type="submit"
          disabled={!connected || !jettonBalance || jettonAmount === 0 || !address}
        >
